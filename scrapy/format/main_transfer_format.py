...
def getFromMongo():
    ...
    if schedual_id[0] >= 39999:
        return
    c = {'id': schedual_id[0]}
    ips = col.find({'$or':[{'status':{'$exists':False}},{'status':None}]}, {'ip': 1}).distinct('ip')  # 去重
    if ips.__len__() == 0: return
    print 'ips length: ', ips.__len__()
    ipsArr = substrIps(ips)
    while True:
        ipsPage = ipsArr[skip:(skip + pageSize)]
        if ipsPage.__len__() == 0: break
        skip = skip + pageSize
        insertIPWare(ipsPage, 3, counter, c)
    #更新有问题，所以一次更新
    col.update_many({'ip':{'$in':ips}},{'$set':{'status':1}})
