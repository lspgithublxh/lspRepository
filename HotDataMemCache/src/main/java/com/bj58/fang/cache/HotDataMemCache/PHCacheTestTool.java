...
public class PHCacheTestTool<T> extends Application{
	...
	public static void main(String[] args) {
		try {
			...
			//无缓存时占用内存：
			PuHotDataCache<String, String> cache = new PuHotDataCache<String, String>(new IGetValByKey<String, String>() {
				@Override
			...
			conf.setStatUnit(500 * 50);//平均速率:1/50ms  而20ms请求一次，共1000条， 那么每条的平均速率1/50ms 所以有50%的会被移除
...
//			conf.setUpdatePerid(1000);
			...
			System.out.println("initail use:" + (m1 - m2));
	...
	private PuHotDataCache<String,T> source = null;
	
	@SuppressWarnings("rawtypes")
	private static PuHotDataCache quan = null;
	
	public void testCache(PuHotDataCache<String, T> source) {
		this.source = source;
		quan = source;
		launch(new String[] {});
	}
