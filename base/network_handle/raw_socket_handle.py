...
ip_source = '10.252.24.248'  # 本机IP
ip_dest = '10.252.24.248'  # 也可以用域名：socket.gethostbyname('www.microsoft.com')64.4.11.42 144.48.5.62
...
ip_saddr = socket.inet_aton( ip_source)  # 两边的ip地址
ip_daddr = socket.inet_aton( ip_dest)
# socket.inet_ntoa() inet_pton  socket.AF_INET,  socket.AF_INET,
...
tcp_flags = tcp_flag_fin + (tcp_flag_syn << 1) + (tcp_flag_rst << 2) + (tcp_flag_psh << 3) + (tcp_flag_ack << 4) + (
        tcp_flag_urg << 5)

# 按上面描述的结构，构建tcp header。
tcp_header = pack('!HHLLBBHHH', tcp_sport, tcp_dport, tcp_seq, tcp_ack_seq, tcp_offset_reserv, tcp_flags,
...
# 发送出去
print 'send the packet:', packet
s.sendto(packet, (ip_dest, 0))
rs = s.recvfrom(2048)
content = rs[0]
print 'content:'
print content.decode('UTF-8', errors='replace')
print 'receive th packet:', rs